...
var utils = (function() {
	...
	_utils.prototype = {
		 ...
		 */
		publishProgress:function (current,total,id) {
//			console.log("current= " + current + "******total= " + total);
			if(current<total&&id!=''){
				//上传图标的透明度变化效果
				if(id == "btn-import"){
					var alpha =$("#btn-import i").css("opacity")==1? 0.5:1;
			  		$("#btn-import i").css("opacity",alpha);
			  		//显示进度
			  		$("#btn-import span").text(current + "/" + total);
			  		var newWidth = 105*current/total + 'px';
					//动态填充按钮
			  		$("#btn-import").css("width",newWidth);
				}if(id == "btn-import-record"){
					var alpha = $("#btn-import-record i").css("opacity")==1? 0.5:1;
			  		$("#btn-import-record i").css("opacity",alpha);
			  		//显示进度
			  		$("#btn-import-record span").text(current + "/" + total);
			  		var newWidth = 105*current/total + 'px';
					//动态填充按钮
			  		$("#btn-import-record").css("width",newWidth);
					
				}if(id == "btn-import-jh"){
					var alpha = $("#btn-import-jh i").css("opacity")==1? 0.5:1;
			  		$("#btn-import-jh i").css("opacity",alpha);
			  		//显示进度
			  		$("#btn-import-jh span").text(current + "/" + total);
			  		var newWidth = 120*current/total + 'px';
					//动态填充按钮
			  		$("#btn-import-jh").css("width",newWidth);
				}if(id == "btn-import-cw"){
					var alpha = $("#btn-import-cw i").css("opacity")==1? 0.5:1;
			  		$("#btn-import-cw i").css("opacity",alpha);
			  		//显示进度
			  		$("#btn-import-cw span").text(current + "/" + total);
			  		var newWidth = 120*current/total + 'px';
					//动态填充按钮
			  		$("#btn-import-cw").css("width",newWidth);
					
				}if(id == "btn-import-pf"){
					var alpha = $("#btn-import-pf i").css("opacity")==1? 0.5:1;
			  		$("#btn-import-pf i").css("opacity",alpha);
			  		//显示进度
			  		$("#btn-import-pf span").text(current + "/" + total);
			  		var newWidth = 120*current/total + 'px';
					//动态填充按钮
			  		$("#btn-import-pf").css("width",newWidth);
				}
			}
		},
		//判断当前浏览器是否为IE9或IE8
		getIsIE9:function(){
		    ...
		    if (browser == "Microsoft Internet Explorer" && (trim_Version == "MSIE9.0"||trim_Version == "MSIE8.0")) {
				return true;	
		    } else {
		    	return false;
		    }
...
function Map() {
	...
	this.setItem = function(key, value) {
		...
		if(!(value instanceof Map)) {
			...
			} catch(e) { console.log('value不是MAP对象') }
		}
	};
	//获取键对应的值
